...
namespace SYS_SGI.Presentation.Controllers.MOV
{
    ...
    public class IndicadorEjecucionController : BaseController
    {
        ...
        public ActionResult Mantenimiento(int codigoAlineamientoEstrategico = 0, string fechaPeriodo = "")
        {
            ...
            vmIndicadorDetalle.IndicadorDetalle = _IndicadorDetalleAppService.Listar(codigoAlineamientoEstrategico, Enums.TipoRegistro.Ejecucion, Enums.TipoPlan.PlanEjecucion, fechaPeriodo).FirstOrDefault();//.Where(x=>x.CodigoAlineamientoEstrategico==codigoAlineamientoEstrategico && x.FechaPeriodoDesc==fecha).FirstOrDefault();
            ...
            foreach (var item in matches)
            {
                ...
                vmIndicadorDetalle.IndicadorDetalle.listaVariableFormula.Add(new VariableFormulaLogic   { CodigoVariable = codigoVariable ,
        ...
        public JsonResult Guardar(VMIndicadorDetalle formMantenimiento)
        {
            if (base.ModelState.IsValid)
            {
                ...
                if (formMantenimiento.IndicadorDetalle.CodigoIndicadorDetalle == 0)
                    appResponse = _IndicadorDetalleAppService.MantenimientoNuevo(new List<IndicadorDetalleLogic> { formMantenimiento.IndicadorDetalle }, new List<VariableFormulaLogic>());
        ...
        public ActionResult Paginacion(string arg, string codigoGuiaEmpresarial)
        {
            ...
            {
                data = _IndicadorDetalleAppService.Paginacion(paginateParams, codigoGuia, Enums.TipoRegistro.Ejecucion, Enums.TipoPlan.PlanEjecucion);
                if (data.Count > 0)
        ...
        public FileResult DescargarReporte(int codigoTipoGuiaEmpresarial, int codigoAlineamientoEstrategico, string indicador)
        {
            ...
            string rutaCompleta = CrearReporte(dataValores, Regex.Replace(indicador, @"[\d-\.]", string.Empty), cantLineamientos);
        ...
        public DataTable ObtenerValores(DataTable dataLineamiento, int codigoAlineamientoEstrategico)
        {
            ...
            {
                listaIndicadorDetalleMeta = _IndicadorDetalleAppService.Listar(codigoAlineamientoEstrategico, Enums.TipoRegistro.Meta, Enums.TipoPlan.PlanEjecucion, "").Where(x => x.Estado).ToList();
                listaIndicadorDetalleEjecucion = _IndicadorDetalleAppService.Listar(codigoAlineamientoEstrategico, Enums.TipoRegistro.Ejecucion, Enums.TipoPlan.PlanEjecucion, "").Where(x => x.Estado).ToList();
            }
