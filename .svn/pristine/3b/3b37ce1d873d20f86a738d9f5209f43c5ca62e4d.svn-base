(function (a, b) {
    ...
    a.BUTTON_SIZES[a.SIZE_GMD] = "btn-lg";
    a.ICON_SPINNER = "glyphicon glyphicon-asterisk";
    a.ZINDEX_BACKDROP = 1000;
    a.ZINDEX_MODAL = 1010;
    a.defaultOptions = {
        id: "",
        type: a.TYPE_PRIMARY,
    ...
    a.openAll = function () {
        ...
        initOptions: function (c) {
            ...
            if (this.isRealized()) {
                var c = this; this.getModal().removeClass(a.SIZE_NORMAL).removeClass(a.SIZE_WIDE).removeClass(a.SIZE_LARGE).removeClass(a.SIZE_GMD);
                this.getModal().addClass(this.getSize());
                this.getModalDialog().removeClass("modal-lg");
                if (this.getSize() === a.SIZE_WIDE) {
                    this.getModalDialog().addClass("modal-lg")
                }
                if (this.getSize() === a.SIZE_GMD) {
                    this.getModalDialog().addClass("modal-gmd")
                }
        ...
        getDescription: function () {
            ...
            //c.prepend(this.createSaveButton()); // se creo el boton guardar
            c.prepend(this.createCloseButton());
            return c
        }, createTitleContent: function () {
            var c = b("<div></div>"); c.addClass(this.getNamespace("title"));
            return c
        }, createSaveButton: function () {// se creo el boton guardar
            var d = b("<div></div>");
            d.addClass(this.getNamespace("save-button"));
            var c = b('<button id="btnGrabarModal" title="Guardar" class="glyphicon glyphicon-floppy-disk" style="cursor:pointer;background:0 0;border:0;color:white;"></button>');
            d.append(c);
            d.on("click", {
                dialog: this
            }, function (e) {                
                //var div = $('#' + this.nextElementSibling.id.replace('_title', '')).find('form')[0].id
                //$("#" + div).submit();
            ...
            //var c = b('<button title="Cancelar" class="glyphicon glyphicon-ban-circle" style="-webkit-appearance:none;cursor:pointer;background:0 0;border:0;color:red;"></button>');
            ...
            d.addClass(this.getNamespace("footer-buttons pull-right"));
            this.indexedButtons = {};
